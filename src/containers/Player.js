...
import TracksList from 'C:/Users/User/Desktop/MyPlayer/myplayer/src/components/TracksList.js';
import DB from 'C:/Users/User/Desktop/MyPlayer/myplayer/src/assets/js/db.js';
...
class Player extends Component {
    ...
    toggleSidebar = () => {                           // открытие боковой панели
        this.setState({sidebar: !this.state.sidebar});
    }

    toggleLoop = () => {                                  //переключение
        const player = this.state.player;
        player.loop = !this.state.loop
        this.setState({player: player, loop: !this.state.loop});
	}
    
    toggleShuffle = () => {                // случайный порядок воспроизведения песен
        this.setState({shuffle: !this.state.shuffle});
    }
    
	playNextTrack = () => {                      // включение следующей песни
	...
	playPrevTrack = () => {                                    // включение предыдущей песни
		let currentTrackIndex = this.state.currentTrackIndex;
		if (!this.state.shuffle) {
			currentTrackIndex -= 1;
    ...
    selectThisTrack = (event, trackID) => {                 // выбор песни
    ...
    updatePlayingInfo = () => {                        // обновление данных о песне каждый раз 
        const player = this.state.player;
    ...
    updateProgressBar = () => {                   // обновление бегунка песни по её длине
    ...
    playPauseAudio = () => {             // стоп / плей песни 
    ...
    onPlayPauseClickHandler = (event) => {         // обработка кликов от пользователя 
        this.setState(
    ...
    progBarPickHandler = (event) => {                   // обработка кликов от пользователя 
    ...
    progBarMoveHandler = (event) => {            // функция для перемещения бегунка пользователем
    ...
    volumeBarPickHandler = (event) => {                      // функция изменения громкости пользователем 
        if (event.type === 'mousedown') {
            this.touchPositionFst = event.pageX;
    ...
    loadTrackIntoState = () => {        // загрузка трека из плейлиста
